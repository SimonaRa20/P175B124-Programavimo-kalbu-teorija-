...
object ControlFunction
{
    def forMaster(bot: Bot) {
        ...
        val nextToMine = bot.inputAsIntOrElse("nextToMine", 0)
        
        
        val bfs = PathFinder(bot, bot.view)
        if(XY(0,0) != bfs){
            bot.move(bfs)
        }
        else
    ...
    def reactAsLandmine(bot: MiniBot) {
        ...
        for (newXY <- PathFinder.getNeighbours(XY(0,0), 5); newXY2 <- PathFinder.getNeighbours(newXY, 5))
        {
    ...
    def create = (input: String) => {
        ...
        opcode match {
            case "React" => {
                ...
                //"Log(text=reacting "+params("energy")+")"
            }
            case _ => {
                ""
            }
...
object PathFinder {
    ...
    def apply(bot: Bot, view: View) : XY = {
        ...
        while(!queue.isEmpty){
            val currentXY = queue.dequeue()             // grab the first element in queue
            
            for ( newXY <- getNeighbours(currentXY, viewRange))
            {
                val cell = view(newXY) // get the cell value
                
                if (cell != 'W' && cell != 'b' && cell != 'p' && cell != 's' && cell != 'm'){
    ...
    def getNeighbours(start: XY, range: Int) : Set[XY] =
    {
        ...
        for (x <- -1 to 1; y <- -1 to 1){           // everyithing in 3x4
            if (!(x == 0 && y == 0)){               // excluding middle
                val newXY = start + XY(x,y)
                // excluding out of range
                if (newXY.x > -range && newXY.x < range && newXY.y > -range && newXY.y < range){
                    neighbours += newXY
                }
            }
        }
